...
### Added
- Runtime Redis Sentinel Rejoin Fencing Evidence v5.2 adds Redis 8 + three-Sentinel quorum CI evidence that, after automatic promotion, the restarted old primary is reconfigured as a connected read-only replica of the promoted authority, direct writes to the returned node are rejected, application discovery remains on the promoted master, and the replicated limiter budget remains coherent while asymmetric-partition split-brain prevention, external fencing/STONITH, Redis Cluster, deployment-scale load/soak, production RPO/RTO, live Unreal recovery, and production HA/DR remain open
...
- Runtime Reference Race Operation Parity v4.2 centralizes start/checkpoint/finish operation-id replay payload equality in Go core, binds PostgreSQL replay handling to the shared contract, and adds executable Python/Go vectors while keeping full reference-oracle parity and packaged Unreal retry evidence open
- Runtime Reference Race Start Parity v4.1 centralizes deterministic Roadworthy/active-build race-start eligibility in the Go core, binds the PostgreSQL race-start transaction to that validator after authoritative ownership/build resolution, and adds shared Python/Go vectors while keeping full reference-oracle parity and live packaged Unreal gates open
...
- Runtime Reference Entitlement Parity v3.6 shared Python/Go vectors for entitlement-bounded garage capacity and routine starter-lineage deletion protection without coupling VIP capacity to competitive build/race outcomes
...
- Unreal Engine 5.8 C++ project with Game / Editor / dedicated Server targets
...
### Changed
- Redis recovery evidence governance now records v5.2 as Sentinel old-primary rejoin/read-only demotion CI evidence while keeping asymmetric-partition split-brain prevention, external fencing/STONITH, Redis Cluster/deployment HA, production RPO/RTO, load/soak, live Unreal recovery, and production readiness open
...
- Extended post-baseline reference parity/hardening through v4.5 with shared build/quest/race-input/race-result vectors, fixture integrity, payload-bound idempotency, Garage 17 rebuild semantics, deterministic build-bound final-result hashing, entitlement capacity/starter-lineage deletion invariants, canonical MQ001–MQ100 identifier/immediate-prerequisite sequencing, server-derived quest reward parity, removal of caller-controlled reward amounts from the Python quest-completion oracle, shared checkpoint/finish lifecycle-order acceptance vectors, shared race-start Roadworthy/active-build binding vectors, start/checkpoint/finish operation-id replay payload-binding vectors, deterministic MQ004/MQ005/MQ009/MQ012 inventory/blueprint/Roadworthy side-effect vectors, rebuild operation replay owner/vehicle/source-result revision/hash binding, and authoritative-character quest durable-operation scoping while keeping full reference-oracle parity, live Unreal race transport, real UE build/package, and production gates open
...
- Redis replica-promotion + Sentinel evidence covers expiring limiter coordination state, automatic Sentinel master rediscovery after old-primary stop, and v5.2 read-only demotion/rejoin of a restarted old primary; asymmetric-partition split-brain prevention, external fencing/STONITH, Redis Cluster, replication-loss bounds, deployment-scale load/soak, and production Redis HA remain unproven
