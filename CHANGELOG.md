...
- Advanced PostgreSQL observability through v3.1-v3.3 with bounded query-activity metrics, an external exporter process, and a CI-proven dedicated `pg_monitor` principal while keeping `pg_stat_statements`, production secret-manager delivery/rotation, deployed scrape/dashboard/SLO evidence, load/soak, HA/DR, and production gates open
