...
def main() -> None:
    ...
    require(docs_index, "runtime-reference-operation-parity-v2.6.md", "docs index operation parity v2.6")
    require(docs_index, "runtime-reference-rebuild-parity-v2.7.md", "docs index rebuild parity v2.7")
    ...
    require(docs_index, "runtime-quest-store-replay-binding-v4.8.md", "docs index quest store replay binding v4.8")
    require(changelog, "Runtime Quest Store Replay Binding v4.8", "CHANGELOG quest store replay binding v4.8")
    require(checklist, "quest store replay binding v4.8", "checklist quest store replay binding v4.8")
    require(docs_index, "runtime-postgres-transaction-failover-v4.9.md", "docs index PostgreSQL transaction failover v4.9")
    require(changelog, "Runtime PostgreSQL Transaction-Boundary Failover Evidence v4.9", "CHANGELOG PostgreSQL transaction failover v4.9")
    require(checklist, "PostgreSQL transaction-boundary failover recovery v4.9", "checklist PostgreSQL transaction failover v4.9")
    require(docs_index, "runtime-redis-promotion-recovery-v5.0.md", "docs index Redis promotion recovery v5.0")
    require(changelog, "Runtime Redis Promotion Recovery Evidence v5.0", "CHANGELOG Redis promotion recovery v5.0")
    require(checklist, "Redis replica-promotion limiter recovery v5.0", "checklist Redis promotion recovery v5.0")
    require(docs_index, "runtime-redis-sentinel-failover-v5.1.md", "docs index Redis Sentinel failover v5.1")
    require(changelog, "Runtime Redis Sentinel Failover Evidence v5.1", "CHANGELOG Redis Sentinel failover v5.1")
    require(checklist, "Redis Sentinel master-discovery failover v5.1", "checklist Redis Sentinel failover v5.1")
    require(docs_index, "runtime-redis-sentinel-rejoin-v5.2.md", "docs index Redis Sentinel rejoin v5.2")
    require(changelog, "Runtime Redis Sentinel Rejoin Fencing Evidence v5.2", "CHANGELOG Redis Sentinel rejoin v5.2")
    require(checklist, "Redis Sentinel old-primary rejoin/read-only fencing v5.2", "checklist Redis Sentinel rejoin v5.2")
    require(checklist, "[ ] Redis asymmetric-partition split-brain prevention/Cluster/deployment-HA evidence", "Redis split-brain/deployment-HA gate remains open")
