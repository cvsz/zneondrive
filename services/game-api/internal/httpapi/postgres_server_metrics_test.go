...
func TestPostgresServerMetricsExposeBoundedNumericStats(t *testing.T) {
	provider := fakePostgresServerStatsProvider{stats: store.PostgresServerStats{
	...
	for _, forbidden := range []string{"postgres://", "DATABASE_URL", "SELECT ", "zneondrive_test", "account", "vehicle", "race"} {
...
func TestPostgresServerMetricsFailureDoesNotExposeError(t *testing.T) {
	provider := fakePostgresServerStatsProvider{err: errors.New("dial postgres://secret@db.internal/zneondrive_private")}
	h := NewPostgresServerMetricsHandler(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {}), provider)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/metrics", nil))
	body := rec.Body.String()
	if !strings.Contains(body, "zneondrive_postgres_up 0") {
		t.Fatalf("expected failed PostgreSQL probe metric: %s", body)
	}
	for _, forbidden := range []string{"secret", "db.internal", "zneondrive_private", "dial postgres"} {
		if strings.Contains(body, forbidden) {
